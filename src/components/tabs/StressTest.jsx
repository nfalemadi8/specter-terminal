...
const SCENARIOS = [
  { id: 'crash2008', name: '2008 Financial Crisis', desc: 'S&P 500 -38.5%, credit freeze, bank failures', marketShock: -38.5, sectorShocks: { Financials: -55, Technology: -40, 'Consumer Discretionary': -42, Energy: -35, Healthcare: -25, 'Consumer Staples': -18 } },
  { id: 'covid2020', name: 'COVID-19 Crash (Mar 2020)', desc: 'S&P 500 -33.9%, pandemic shutdown', marketShock: -33.9, sectorShocks: { Energy: -52, Financials: -38, 'Consumer Discretionary': -35, Technology: -25, Healthcare: -20, 'Consumer Staples': -15 } },
  { id: 'dotcom', name: 'Dot-Com Bust (2000-02)', desc: 'S&P 500 -49.1%, tech bubble burst', marketShock: -49.1, sectorShocks: { Technology: -72, 'Communication Services': -65, 'Consumer Discretionary': -35, Financials: -20, Healthcare: -15, Energy: -10, 'Consumer Staples': -8 } },
  { id: 'rateshock2022', name: '2022 Rate Shock', desc: 'Fed 425bp hikes, S&P -19.4%', marketShock: -19.4, sectorShocks: { Technology: -28, 'Real Estate': -25, 'Consumer Discretionary': -20, Financials: 5, Energy: 3, 'Consumer Staples': -8, Healthcare: -12 } },
  { id: 'blackmonday', name: 'Black Monday 1987', desc: 'Single-day crash -22.6%', marketShock: -22.6, sectorShocks: { Technology: -25, Financials: -28, 'Consumer Discretionary': -22, Energy: -18, Healthcare: -15, 'Consumer Staples': -12 } },
  { id: 'gccoilcrash', name: 'GCC Oil Crash 2014', desc: 'Oil $115→$28, Gulf fiscal crisis', marketShock: -5, sectorShocks: { Energy: -48, Financials: -15, 'Real Estate': -25, Technology: -3, 'Consumer Staples': -2 } },
  { id: 'stagflation', name: 'Stagflation 1973', desc: 'OPEC embargo, S&P -48%, gold +73%', marketShock: -48, sectorShocks: { Technology: -30, Financials: -28, 'Consumer Discretionary': -32, Energy: 10, 'Consumer Staples': -10, Healthcare: -15 } },
  { id: 'oilshock', name: 'Oil Crisis ($150/bbl)', desc: 'Oil spikes to $150, inflation surge', marketShock: -15, sectorShocks: { Energy: 25, Technology: -18, 'Consumer Discretionary': -22, 'Consumer Staples': -12, Financials: -15 } },
  { id: 'geopolitical', name: 'Geopolitical Escalation', desc: 'Major conflict, trade disruption', marketShock: -20, sectorShocks: { Technology: -25, Energy: 15, Financials: -18, 'Consumer Discretionary': -22, 'Consumer Staples': -8 } },
  { id: 'gccreboom', name: 'GCC Real Estate Boom', desc: 'RE +40%, oil +15%', marketShock: 2, sectorShocks: { 'Real Estate': 40, Financials: 12, 'Consumer Discretionary': 8, Energy: 15, Technology: 3 } },
];
...
function runScenario(scenario, holdings, stockData) {
  const detailed = holdings.map(h => {
    ...
    return { ...h, beta, shock: adjustedShock * 100, stressedPrice, stressedValue, pnl, pnlPct: adjustedShock * 100, currentValue };
  });
  ...
  return {
    holdings: detailed.sort((a, b) => a.pnl - b.pnl), totalCurrent, totalStressed, totalPnl, totalPnlPct, var95, var99, maxDrawdown, avgBeta,
...
function StressTest() {
  ...
  useEffect(() => { getAllStocks().then(d => setStocks(d || [])); }, []);

  const scenario = useMemo(() => {
    if (useCustom) return { id: 'custom', name: 'Custom Scenario', desc: `Market ${customShock >= 0 ? '+' : ''}${customShock}%`, marketShock: customShock, sectorShocks: customSectors };
    return SCENARIOS.find(s => s.id === selectedScenario) || SCENARIOS[0];
  }, [selectedScenario, useCustom, customShock, customSectors]);

  const results = useMemo(() => runScenario(scenario, portfolioHoldings, stocks), [scenario, stocks]);
  ...
  const drawdownPath = useMemo(() => {
    ...
    return Array.from({ length: steps + 1 }, (_, i) => {
      const t = i / steps;
      const phase = t < 0.4 ? t / 0.4 : 1;
      const recovery = t > 0.6 ? (t - 0.6) / 0.4 * 0.3 : 0;
      const change = shock * phase + recovery * Math.abs(shock);
      return { day: i, value: results.totalCurrent * (1 + change) };
    });
    ...
    <div className="h-full grid grid-cols-12 grid-rows-6 gap-[3px] p-[3px]">
      <Panel title="PORT SCEN — Scenario Selection" className="col-span-3 row-span-4">
        <div className="space-y-1 text-[10px]">
          <div className="flex gap-1 mb-1">
            <button onClick={() => setUseCustom(false)} className={`flex-1 py-0.5 border text-[9px] ${!useCustom ? 'border-bb-amber text-bb-amber bg-bb-amber/10' : 'border-bb-border text-bb-muted'}`}>Presets</button>
            <button onClick={() => setUseCustom(true)} className={`flex-1 py-0.5 border text-[9px] ${useCustom ? 'border-bb-amber text-bb-amber bg-bb-amber/10' : 'border-bb-border text-bb-muted'}`}>Custom</button>
          </div>
          {!useCustom ? (
            <div className="space-y-1 overflow-auto" style={{ maxHeight: 'calc(100% - 30px)' }}>
              {SCENARIOS.map(sc => (
                <button key={sc.id} onClick={() => setSelectedScenario(sc.id)}
                  className={`w-full text-left p-1.5 border transition-colors ${selectedScenario === sc.id ? 'border-bb-red bg-bb-red/5' : 'border-bb-border hover:border-bb-muted'}`}>
                  <div className={`font-bold text-[10px] ${selectedScenario === sc.id ? 'text-bb-red' : 'text-bb-white'}`}>{sc.name}</div>
                  <div className="text-bb-muted text-[8px] mt-0.5">{sc.desc}</div>
                  <div className={`text-[9px] mt-0.5 font-bold ${colorClass(sc.marketShock)}`}>Market: {sc.marketShock > 0 ? '+' : ''}{sc.marketShock}%</div>
                </button>
              ))}
            </div>
          ) : (
            <div className="space-y-2">
              <div>
                <div className="flex justify-between text-bb-muted mb-0.5"><span>Market Shock</span><span className={colorClass(customShock)}>{customShock}%</span></div>
                <input type="range" min={-60} max={30} value={customShock} onChange={e => setCustomShock(+e.target.value)} className="w-full h-1 appearance-none bg-bb-border rounded cursor-pointer accent-[#ff3b3b]" />
              </div>
              <div className="border-t border-bb-border pt-1">
                ...
                {sectors.map(sec => (
                  <div key={sec} className="flex items-center justify-between mb-1">
                    ...
                    <input type="number" value={customSectors[sec] ?? ''} placeholder={customShock.toString()}
                      onChange={e => setCustomSectors(prev => ({ ...prev, [sec]: e.target.value === '' ? undefined : +e.target.value }))}
      ...
      <Panel title="Risk Metrics" className="col-span-2 row-span-2">
        <table className="bb-table"><tbody>
          {[
            ...
            ['Max DD', round(results.maxDrawdown, 1) + '%', 'text-bb-red'],
      ...
      <Panel title="Scenario Drawdown Path" className="col-span-3 row-span-2">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={drawdownPath} margin={{ top: 5, right: 10, bottom: 5, left: 5 }}>
            ...
            <XAxis dataKey="day" tick={{ fill: '#6a6a6a', fontSize: 8 }} />
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={comparison} margin={{ top: 5, right: 5, bottom: 5, left: 5 }} layout="vertical">
            ...
            <Bar dataKey="totalPnlPct" radius={[0, 2, 2, 0]}>
              {comparison.map(c => <Cell key={c.name} fill={c.totalPnlPct >= 0 ? '#00d26a' : '#ff3b3b'} />)}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </Panel>
      ...
      <Panel title="Per-Holding Stress Impact" className="col-span-6 row-span-4">
        <table className="bb-table">
          <thead>
            <tr><th>Symbol</th><th>Sector</th><th className="text-right">Shares</th><th className="text-right">Current</th><th className="text-right">Stressed</th><th className="text-right">Beta</th><th className="text-right">Shock</th><th className="text-right">P&L</th><th className="text-right">P&L %</th></tr>
          </thead>
          <tbody>
          ...
          <ResponsiveContainer width="100%" height={80}>
            <BarChart data={results.holdings} margin={{ top: 2, right: 5, bottom: 0, left: 5 }}>
              ...
              <Bar dataKey="pnl" radius={[2, 2, 0, 0]}>
                {results.holdings.map(h => <Cell key={h.symbol} fill={h.pnl >= 0 ? '#00d26a' : '#ff3b3b'} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="space-y-0.5 text-[10px] p-0.5">
          {Object.entries(scenario.sectorShocks || {}).sort((a, b) => a[1] - b[1]).map(([sector, shock]) => (
            <div key={sector} className="flex items-center justify-between">
              ...
              <span className={`w-10 text-right font-bold ${colorClass(shock)}`}>{shock > 0 ? '+' : ''}{shock}%</span>
            </div>
          ))}
        </div>
      </Panel>
